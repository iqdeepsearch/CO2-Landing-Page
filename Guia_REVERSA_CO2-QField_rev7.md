# Guia de Aplicação do Framework REVERSA para o Projeto CO₂-QField (Rev 7)

> **Nota de revisão (Rev 7):** Ajuste de nomenclatura em espelho ao Memorial Descritivo `CO2-QField_rev7.md` — a role de escrita bruta por planta passou a se chamar `field_ingest_{planta_id}` (antes `qfield_{planta_id}`), e a captura do nome do operador é referida como regra de payload, não mais como expressão específica do QField. Nenhuma etapa, agente ou sequência do REVERSA foi alterada.

Para criar o aplicativo de monitoramento de emissões de CO₂ industrial baseado no Memorial Descritivo `CO2-QField_rev7.md`, você deve seguir o Ato 2 — Ideação e Concepção Greenfield (Tier 2) do framework REVERSA (https://github.com/sandeco/reversa).  
...
* **reversa-brainstorm:** Inicie aqui para orquestrar a sessão de ideação do projeto CO₂-QField. Esta função irá preparar o terreno para alinhar o escopo da solução geoespacial, a governança de dados e os requisitos regulatórios ambientais de cálculo.  
* **reversa-framer:** Fundamental para diagnosticar o "problema raiz". Aqui você definirá a dor real da empresa (gestão ambiental manual, risco de não-conformidade com o GHG Protocol/ISO 14064 e lentidão em auditorias) em contrapartida à solução proposta (automação via QR Code, GPS, cálculo preliminar visual, persistência em banco relacional e atalho para dashboard web).  
...
* **reversa-arbiter:** O juiz que tomará a decisão sobre a arquitetura (Stack consolidada: QGIS, QField, Supabase/PostgreSQL com PostGIS, tabelas versionadas de fatores de conversão, motor Python para cálculo oficial em Escopos 1 e 2, e Dashboard Web em HTML/JS/Leaflet).  
...
* **Aplicação no seu caso:** Separe em tarefas como:
  ...
  3. Parametrização do QField/QFieldCloud (cliente de campo atual — ver Seção 5.6) em modo *Offline Editing* e teste de rotina *offline-first*;
  4. Desenvolvimento do motor de cálculo oficial em Python (`pandas`/`sqlalchemy`), executado sob a role `calc_engine`, implementando o contrato de resolução dinâmica do fator vigente (`ORDER BY vigencia_inicio DESC LIMIT 1`, Seção 5.2) e a transição de status `rascunho → calculado` via trigger de banco;
  5. Construção da tela/fluxo de validação e exportação para a role `gestor_auditor` (transições `calculado → validado → exportado`);
  6. Construção e integração do Dashboard Web interativo com Leaflet.js e gráficos analíticos de conformidade.
